...
library(ggplot2)
df <- data.frame("x"= dots_x, "y"= dots_y)
ggplot(df, aes(x,y)) + geom_point() + geom_segment(aes(x,y, xend=0, yend=0)) + coord_quickmap() + 
  labs(title = "Relation between Origin and Destination in 100 seconds", x="X Position", y="Y Position")